...
### Object Equality
**IMPORTANT** : Objects are compared by **reference** not by **contents**.  What does that mean for us?

It means that two distinct, different, objects are never directly equal to each other *even* if they have the same contents.
...
const areEqual = obj1 === obj2; // Returns false!
```

In order to determine not if Objects are equal, but if they contain the same properties and values, we need to write more complex logic that recursively compares them or rely on a third party module.